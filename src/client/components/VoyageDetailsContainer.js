...
export default class VoyageDetailsContainer extends Component {
  ...
  async getVesselReport(voyageId) {
    return new Promise((resolve, reject) => {
      axios
        .get(
        ...
        .then(response => {
          resolve(camelcaseKeys(response.data)); // get all vessel reports
        })
        .catch(error => reject(error));
    });
  }
    ...
    this.getVoyages(this.state.organizationId)
      ...
      .then(() => {
        ...
        this.state.voyages.map(voyage => {
          this.getVesselReport(voyage.id).then(vesselReport => {
            this.setState({
  ...
  render() {
    return (
      <div>
        <div>This page is about the Voyage ID = {match.params.voyage_id}</div>;
        <MapComponent
          vesselReports={this.state.vesselReports}
          suggestedRoutes={this.state.suggestedRoutes}
          options={mapOptions}
        />
      </div>
    );
